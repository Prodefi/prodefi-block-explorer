...
import types from "../../bus/modal/types";
...
@translate()
class ValidatorProfile extends PureComponent {
  state = {
    tabIndex: 0,
    isMounted: false,
  ...
  componentWillReceiveProps(newProps) {
    ...
    if (isMounted) {
      ...
      const seek = path.indexOf('validator-profile');
  ...
  getData = () => {
    ...
    if (address && address !== '') {
      ...
      if (address.indexOf('pdtvalcons') !== -1 && height && height !== 'latest' && parseInt(height) !== 0) {
        this.props.actions.fetchProposerAddress(address, height).then((res) => {
          if (!!res && !!res.type && res.type === types.FETCHING_PROPOSER_ADDRESS_SUCCESS) {
            this.props.actions.fetchStakingValidatorAddressInfo(res.payload.operator_address);
          }
        });
  ...
  loadData = (address) => {
    if (address) {
      ...
      this.props.history.push(`/validator-profile/${address}/latest`);
    }
  };
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        <Header>
          <NavigationMenu/>
          <PageTitle>Validator Profile</PageTitle>
